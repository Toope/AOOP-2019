...
TEST_CASE("SymbolicSquareMatrix tests","[symbolicsquarematrix]") 
{
    ...
    SymbolicSquareMatrix sym = ssm2 + ssm2;                             //+
    CHECK(sym.toString() == "[[(1+1),(a+a)][(3+3),(b+b)]]");
    REQUIRE(sym.evaluate(map).toString() == "[[2,10][6,40]]");

    
    SymbolicSquareMatrix symmin = sym - ssm2;                           //-
    CHECK(symmin.toString() == "[[((1+1)-1),((a+a)-a)][((3+3)-3),((b+b)-b)]]");
    REQUIRE(symmin.evaluate(map).toString() == "[[1,5][3,20]]");
    
    SymbolicSquareMatrix mutl = ssm2 * ssm2;                            //* , don't test toString() here, looks so messy 
    REQUIRE(mutl.evaluate(map).toString() == "[[16,105][63,415]]");
...
TEST_CASE("CompositeElement tests","[composite_element]") 
{
    Valuation map;
    map['a'] = 5;
    map['b'] = 20;
    ...
    VariableElement a;
    IntElement nol;
    CompositeElement co{a, nol, std::plus<int>{}, '+'};
    CHECK(co.toString() == "(a+0)");
    CHECK(co.evaluate(map) == 5);
    CompositeElement co2{a, nol, std::minus<int>{}, '-'};
    co = co2;
    CHECK(co.toString() == "(a-0)");
    CHECK(co.evaluate(map) == 5); 
    CHECK(co == co2);                                               
...
{
    SymbolicSquareMatrix sq2{"[[3,a][5,6]]"};               //test matrices
    SymbolicSquareMatrix sq3{"[[c,2,7][3,v,8][n,6,m]]"}; 
    
    //check the form of the string like in VT1   
    try
    {
        SymbolicSquareMatrix nope{"[[0,c][3,4][a,x]]"};
    }
    catch(const std::exception& e)
    {
        std::cout << e.what() << std::endl;
        CHECK(std::string{e.what()} == "Not SymbolicSquareMatrix! Rows and cols don't match!");
    }
    ...
    try
    {
        SymbolicSquareMatrix sq_op = sq3 + sq2; 
    }
    catch(const std::exception& e)
    {
        std::cout << e.what() << std::endl;
